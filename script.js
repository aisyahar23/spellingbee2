...
document.addEventListener('DOMContentLoaded', function() {
    // Initialize chatbot first to ensure it works
    initChatbot();
    
    // Initialize other components with error handling
    try { initMobileMenu(); } catch(e) { console.log('Mobile menu error:', e); }
    try { initCharts(); } catch(e) { console.log('Charts error:', e); }
    try { initControlSystems(); } catch(e) { console.log('Control systems error:', e); }
    try { initNotifications(); } catch(e) { console.log('Notifications error:', e); }
    try { initLocationSettings(); } catch(e) { console.log('Location settings error:', e); }
});
...
function initMobileMenu() {
    ...
    if (!header) {
        console.log('Header not found, skipping mobile menu');
        return;
    }
...
function initCharts() {
    ...
    const firstChart = document.querySelector('.chart-placeholder:first-child');
    const lastChart = document.querySelector('.chart-placeholder:last-child');
    ...
    firstChart.appendChild(tempCtx);
    ...
    lastChart.innerHTML = '';
    lastChart.appendChild(resourceCtx);
...
function initChatbot() {
    ...
    function setupQuickActions() {
        ...
        // Also setup existing quick action buttons if they exist
        const existingButtons = document.querySelectorAll('.quick-action-btn[data-query]');
        existingButtons.forEach(button => {
            button.addEventListener('click', () => {
                const query = button.getAttribute('data-query') || button.textContent;
    ...
    function getResponseFromKnowledgeBase(message) {
        // Use the knowledge base helper function
        if (typeof knowledgeBase !== 'undefined' && knowledgeBase.generateResponse) {
            const response = knowledgeBase.generateResponse(message);
        ...
        if (query.includes('tomato')) {
            return "Tomatoes need warm weather and full sun. Plant them after the last frost, space 24-36 inches apart, and provide support with stakes or cages. Water consistently at the base to prevent disease.";
        }
        
        if (query.includes('pest') || query.includes('bug') || query.includes('insect')) {
            return "For pest control, try integrated pest management: use beneficial insects, row covers, crop rotation, and organic sprays like neem oil or insecticidal soap. Identify the specific pest for targeted treatment.";
        }
        
        if (query.includes('soil')) {
            return "Healthy soil is key to successful farming. Test your soil pH (ideal 6.0-7.0 for most crops), add organic matter like compost, and ensure good drainage. Regular soil testing every 2-3 years is recommended.";
        }
        
        if (query.includes('water') || query.includes('irrigation')) {
            return "Most plants need 1-2 inches of water per week. Water deeply and less frequently to encourage deep roots. Morning watering is best to reduce evaporation and disease risk. Consider drip irrigation for efficiency.";
        }
        
        if (query.includes('organic')) {
            return "Organic farming focuses on building soil health naturally. Use compost, cover crops, beneficial insects, and crop rotation. Avoid synthetic fertilizers and pesticides in favor of natural alternatives.";
        }
        
        if (query.includes('plant') || query.includes('grow')) {
            return "Successful planting depends on timing, soil preparation, and proper spacing. Know your frost dates, prepare soil with organic matter, and follow seed packet instructions for depth and spacing.";
        }
        
        // Fallback response
        return "I can help with farming questions about crops, soil, pests, watering, and organic methods. What specific topic would you like to know about?";
    }
